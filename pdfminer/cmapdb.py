...
from .psparser import KWD, PSEOF, PSLiteral, PSStackParser, PSSyntaxError, literal_name
...
class CMapParser(PSStackParser):
    ...
    def do_keyword_endbfrange(self):
        objs = [obj for (__, obj) in self.popall()]
        for (s, e, code) in choplist(3, objs):
            ...
            if not isinstance(code, list):
                ...
                code = (
                    prefix + struct.pack(">L", base + i)[-vlen:]
                    for i in range(e1 - s1 + 1)
                )
            for i, _code in zip(range(s1, e1), code):
                self.cmap.add_cid2unichr(i, _code)

    def popall(self):
        return super().popall()

    KEYWORD_METHOD_MAP = {
        KEYWORD_DEF: do_keyword_def,
        KEYWORD_USECMAP: do_keyword_usecmap,
        KEYWORD_BEGINCODESPACERANGE: popall,
        KEYWORD_ENDCODESPACERANGE: popall,
        KEYWORD_BEGINCIDRANGE: popall,
        KEYWORD_BEGINCIDCHAR: popall,
        KEYWORD_BEGINBFRANGE: popall,
        KEYWORD_BEGINBFCHAR: popall,
        KEYWORD_BEGINNOTDEFRANGE: popall,
        KEYWORD_ENDNOTDEFRANGE: popall,
        KEYWORD_BEGINCMAP: popall,
        KEYWORD_ENDCIDCHAR: do_keyword_endcidchar,
        KEYWORD_ENDBFCHAR: do_keyword_endbfchar,
        KEYWORD_ENDCIDRANGE: do_keyword_endcidrange,
        KEYWORD_ENDBFRANGE: do_keyword_endbfrange,
    }

    def do_keyword(self, pos, token):
        if token is self.KEYWORD_ENDCMAP or not self._in_cmap:
            self._in_cmap = False
            return
        ...
        if keyword_method := self.KEYWORD_METHOD_MAP.get(token):
            keyword_method(self)
            return
...
class CMapConverter:
    ...
    def load(self, fp):
        ...
        for line in fp:
            ...
            def pick(unimap):
                chars = sorted(
                    unimap.items(), key=(lambda x: (x[1], -ord(x[0]))), reverse=True
                )
                (c, _) = chars[0]
                return c
